...
# some of the passwords wouldn't have been allowed by the Official Toboggan Corporate Policy that was
...
for entry in input:
    ...
    password = entry.split(sep=":")[1].strip()
    low = int(entry.split(sep=" ")[0].split("-")[0].strip())
    high = int(entry.split(sep=" ")[0].split("-")[1].strip())
    char = re.search(r'[a-z]', entry, re.I)[0]
...
for entry in input:
    ...
    password = entry.split(sep=":")[1].strip()
    index_1 = int(entry.split(sep=" ")[0].split("-")[0].strip())
    index_2 = int(entry.split(sep=" ")[0].split("-")[1].strip())
    char = re.search(r'[a-z]', entry, re.I)[0]

    if password[index_1-1] == char and password[index_2-1] != char or password[index_1-1] != char and password[index_2-1] == char:
        correct += 1
